...
namespace Monolito_4bm
{
    public partial class ImportarExcel : System.Web.UI.Page
    {
        ...
        {
            string plantilla = "pro_id;pro_nombre;pro_cantidad;pro_precio;prov_id;img_ruta\n;;0;0,00;;\n";
        ...
        protected void btnCargar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!dt.Columns.Contains("pro_nombre") || !dt.Columns.Contains("pro_cantidad") || !dt.Columns.Contains("pro_precio"))
                {
                    MostrarSweetAlert("El archivo debe contener las columnas: pro_nombre, pro_cantidad, pro_precio.", "error");
                    return;
                }
                ...
                {
                    int proId = 0;
                    if (row["pro_id"] != DBNull.Value && int.TryParse(row["pro_id"].ToString(), out proId) && proId > 0)
                    {
                        var prod = productoNegocio.ObtenerProductoPorId(proId);
                        row["Estado"] = prod != null ? "Actualizar" : "Nuevo";
                    }
                    else
                    {
                        row["Estado"] = "Nuevo";
        ...
        protected void btnConfirmar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                productoNegocio.ImportarProductosMasivo(dt, eliminar, actualizar, soloNuevos);

                Session.Remove("DatosExcel");
                pnlPreview.Visible = false;
                pnlCarga.Visible = true;
                MostrarSweetAlert("Productos importados correctamente.", "success", "Productos.aspx");
            }
            catch (Exception ex)
            {
